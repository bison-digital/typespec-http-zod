...
**This line was stale by 19 tests before anyone noticed.** It said 157 while the suite ran 173, and
...
**Nothing is published.** Publishing is public and permanent and needs explicit approval.
...
**This section is why the effort works. Numbers can be re-measured; this cannot be re-derived from
the code.**
...
**`git diff --exit-code` proves nothing about an UNTRACKED file.** A control here reverted a file
...
**A claim in a README is not a guard, and this package shipped one that was not.** The README said
...
**Install the tarball and be the adopter. No harness can see what a harness configures away.** Four
...
**Every one of these was invisible to 173 passing tests, because every harness in both repositories
...
3. **A second differential axis - assessed, prototyped, and deliberately NOT landed.** Everything is
   ...
   **`io: "input"` is load-bearing.** In OUTPUT mode `z.strictObject` and `z.object` both report
   ...
   **`unevaluatedProperties` is `allOf`-AWARE, and that is why openapi3 uses it.** A model
...
5. **Refusal severity is an open question, and the accounting blocks it.** All three refusals are
   ...
   **Do not flip one before re-keying the accounting.** `corpus.ts:244` counts `emitterWarnings` as
