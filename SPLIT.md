...
**The `@typespec/openapi3` precedent does NOT argue against this, and it was cited as though it
...
**The two halves share live in-memory state and a naming contract.** `renderApp` calls
...
`renderRoutes` and everything it emits - `GeneratedRoute`, `GENERATED_ROUTES`,
...
**The library needs a differential of its own, and it cannot be the current one unchanged.** The
...
Nothing is published until step 5 passes. Publishing is public and permanent.
