 ...
 *
 * ⚠️ **Nothing else here asks that.** The differential compares the validator to the document as two
 ...
 *
 * ⚠️ **Every arm is a PAIR.** A test that only checks acceptance passes against `z.any()`, and one
 ...
 *
 * ⚠️ **The module is IMPORTED, which is itself an assertion.** Zod's documented recursion idiom is
...
describe("closed models", () => {
	it("accepts a conformant body and rejects a surplus property", () => {
		/**
		 * ⚠️ **Both directions, because the failure modes are opposite and both have shipped.** Under
...
describe("open and typed shapes", () => {
	it("keeps declared properties checked on a model that also has an indexer", () => {
		/**
		 * ⚠️ **Reading the indexer first emits a dictionary**, which accepts the object and strips every
...
describe("polymorphism", () => {
	...
	it("refuses a variant's body under the wrong discriminator, and an unknown one", () => {
		/**
		 * ⚠️ **A base emitted as its own shape validates none of its polymorphism.** `{kind, label}`
...
describe("constraints", () => {
	...
	it("enforces EXCLUSIVE bounds as exclusive", () => {
		/**
		 * ⚠️ **Collapsing these to inclusive is a real defect with a quiet symptom.** `.min(0)` accepts a
