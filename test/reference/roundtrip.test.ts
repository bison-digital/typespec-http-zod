 ...
 *
 * **Operation identity, not counts.** Comparing `3` against `3` passes while a path is renamed
...
beforeAll(async () => {
	...
	for (const api of discoverReferenceApis()) {
		...
		>;
		/**
		 * **This counted rows in a route TABLE that no longer exists**, and the table was never the
			...
			`${JSON.stringify(
				{
					...
					/**
					 * **How much this loop actually looked at.** The failure lists below are empty, and
...
describe("a published API survives the round trip", () => {
	...
	it("agrees with the round-tripped document about the shapes it declares", () => {
		...
		/**
		 * **Non-vacuity, held to the recorded number rather than to `>= 1`.** The arm above compares
