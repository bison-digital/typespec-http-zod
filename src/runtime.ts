 ...
 *
 * ⚠️ **Kept deliberately tiny, and deliberately free of every TypeSpec import.** The main entry point
 ...
 *
 * ⚠️ **Failure arms are here too, and for a while they were not.** The document declares what an
 ...
 *
 * ⚠️ **The arms arrive in OpenAPI's precedence order, so the FIRST match is the right one.** An exact
 ...
 *
 * ⚠️ **`schema: undefined` is a statement, not an absence.** A response the document says carries no
...
export interface ResponseArm {
	 ...
	 *
	 * ⚠️ **`"default"` is carried as itself, not resolved to a number.** OpenAPI's `default` response
	 ...
	 *
	 * ⚠️ **A range is carried as itself for the same reason, and it used to be dropped entirely.**
 ...
 *
 * ⚠️ **Shipped beside the arms rather than left to the caller, because this is where it goes wrong.**
