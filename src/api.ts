 ...
 * than restate it.
 *
 * ⚠️ **Two schemas that have to agree are two lists that will not.** A server generator adds its own
 ...
 *
 * ⚠️ **Published because a consumer emitting code beside these validators has to spell keys the
 ...
 *
 * ⚠️ **This once named one repository's build script** — `pnpm --filter … generate`, a command that
 ...
 *
 * ⚠️ **`HttpStatusCodesEntry` is `HttpStatusCodeRange | number | "*"`, and the third case was
 ...
 *
 * ⚠️ **A partial range is REFUSED, because `@typespec/openapi3` refuses it.** `494-499` does not
 ...
 *
 * ⚠️ **`"default"` used to be the one arm with no body of its own**, filled in downstream from the
 ...
 *
 * ⚠️ **The specification decides this, not us.** The Responses Object says a range MAY be given as
 ...
 *
 * ⚠️ **`@useAuth(NoAuth)` reaches OpenAPI as `security: []`, which is why this is derivable at all.**
 ...
 *
 * ⚠️ **The emitter does not know what a problem document is, and must not.** This is the general
 ...
 *
 * ⚠️ **This used to be an INVARIANT the emitter enforced, and it is now merely an observation.** It
	...
	for (const operation of service.operations) {
		for (const response of operation.responses) {
			...
			for (const content of response.responses) {
				...
				/**
				 * ⚠️ **An error response can declare no body either.** `tsp-openapi3` renders a bare
 ...
 *
 * ⚠️ **`void` and `never` ARE "no body", wherever they appear.** Answering that in one place is the
 ...
 *
 * ⚠️ **`void` and `never` ARE "no body", and reading them as types was a defect.** A response with
 ...
 *
 * ⚠️ **The fact that decides whether two operations are a collision or a CHOICE.** TypeSpec models
 ...
 *
 * ⚠️ **`requestBodyOf` returns a type only for `bodyKind === "single"`, so every multipart operation
 ...
 *
 * ⚠️ **A binary part is `z.unknown()`, deliberately.** `SCALARS` maps `bytes` to `z.string()`, which
...
export interface EmittedRoute {
	 ...
	 * returning `void`.
	 *
	 * ⚠️ **This used to be required, and requiring it silently deleted routes.** `collectRoutes`
 ...
 *
 * ⚠️ **Three of the four standard collection formats rejected every conformant request**, and only
...
function parameterSchemasOf(
	...
	for (const parameter of operation.parameters.parameters) {
		...
		/**
		 * ⚠️ **`parameter.name` is the WIRE name; `parameter.param.name` is the TypeSpec one.** Headers
		...
		/**
		 * ⚠️ **On a route several operations share, `accept` SELECTS the operation — it is not a
...
export function collectRoutes(
	...
	const routes: EmittedRoute[] = [];
	{
		for (const operation of service.operations) {
			...
			/**
			 * ⚠️ **A streamed operation's EVENT payloads are not reachable from its body.**
			 *
			...
			routes.push({
				 ...
				 *
				 * ⚠️ **This used to be a router's path, converted here.** `toHonoPath` ran during
				...
				/**
				 * ⚠️ **Each position is walked at ITS OWN visibility, which is the whole point.** A model
 ...
 *
 * ⚠️ **The per-operation half used to be emitted by the SERVER generator, into the server file.** So
...
function namedSchema(
	...
	/**
	 * ⚠️ **Exported, because a schema nothing outside this file can name is a schema nothing can
 ...
 *
 * ⚠️ **Keyed on request locations, not on a router's validator targets.** `@hono/zod-validator`
 ...
 *
 * ⚠️ **A DOCUMENT fact, not a router one, which is why it lives here.** TypeSpec models content
 ...
 *
 * ⚠️ **These used to be declared by the SERVER emitter, into the server file.** Six identifiers per
 ...
 *
 * ⚠️ **This docblock was orphaned in the source it came from**, sitting above an unrelated function
...
function responseArmsOf(
	 ...
	 *
	 * ⚠️ **The document declares what an error body looks like and the server never passed it on**, so
	 * `respond` could not check what it sends on failure even in principle.
	 *
	 * ⚠️ **This used to fall back to the service-wide error schema, and the fallback was the defect.**
 ...
 *
 * ⚠️ This is also the only thing that catches the emitter disagreeing with **itself**. `zod.ts` and
 ...
 *
 * ⚠️ **This is the shared seam, and it is deliberately narrow.** It carries no `SchemaRegistry`, no
...
export async function emitHttpZod(
	 ...
	 *
	 * ⚠️ **This used to be a bare `getAllHttpServices`, which is the UNION of every version.** A spec
	...
	/**
	 * ⚠️ **One emission per `@service`, with its own registry and its own import collector.**
	 *
	...
	for (const snapshot of snapshots) {
		...
		const resolved: ResolvedServiceOptions = {
			contractsOutputDir:
				perService?.["contracts-output-dir"] ?? options["contracts-output-dir"],
		...
		const {
			...
			withContractsPackage(resolved.contractsPackage, () =>
				withSealedObjects(resolved.sealObjectSchemas, () => {
					 ...
					 *
					 * ⚠️ **Its expression is registered and then deliberately discarded.** It used to be
		...
		/**
		 * ⚠️ **Named AFTER the walk and BEFORE the file is rendered.** Naming mints identifiers that
