 ...
 *
 * ⚠️ **Reporting does not unwind, which is the point.** A throw abandoned the compile at the first
 ...
 *
 * ⚠️ **A property of the model's own shape, not of where it is used.** This was previously "the
 ...
 *
 * ⚠️ **This mirrors `@typespec/openapi3`'s `shouldSealSchema` deliberately, line for line.** The
 ...
 *
 * ⚠️ **This emitter cannot see the other emitter's options, so the consumer states it twice.**
 ...
 *
 * ⚠️ **Position is a SECOND axis, and this only handles the first.** `@header` is metadata at a
 ...
 *
 * ⚠️ **Not a constant, and no longer defaulted.** Each `@service` publishes its own contracts
 ...
 *
 * ⚠️ **`@externalValues` used to be able to point that arrow the other way** — the spec deferring to
 ...
 *
 * ⚠️ **The union is not a choice between its variant types — it is a choice between wrappings of
...
function unionToZod(program: Program, union: Union): string {
	...
	if (discriminated !== undefined) {
		...
		/**
		 * ⚠️ **Refused, not compensated for.** With no envelope the discriminator lives inside the
 ...
 *
 * ⚠️ **Keyed on the indexer, not on the model's name.** This used to test `name === "Record"`, which
 ...
 *
 * ⚠️ **`model.properties` holds only a model's OWN properties, and `baseModel` appeared nowhere in
...
function modelToZod(program: Program, model: Model): string {
	/**
	 * ⚠️ **A `@discriminator` base is a CHOICE between its subtypes, not a shape of its own.**
	 *
	...
	/**
	 * ⚠️ **HTTP metadata is not payload.** A `@header` or `@statusCode` property describes the envelope,
	 ...
	 */
	/**
	 * ⚠️ **A property that refers back into an unfinished declaration is emitted as a GETTER.**
	 *
	 ...
	 */
	/**
	 * ⚠️ **A deferred property forces the CONSTRUCTOR form of the object.**
	 *
 ...
 *
 * ⚠️ **`@encodedName` is a first-party decorator, and a decorator is a declaration rather than a
 ...
 *
 * ⚠️ **A first-party decorator, and a decorator is a declaration rather than a behaviour** — every
 ...
 *
 * ⚠️ **An array encoding on a QUERY parameter is a style, not a type change — and this docblock used
...
export function propertyToZod(program: Program, property: ModelProperty): string {
	/**
	 * ⚠️ **Constraints go INSIDE `.nullable()`, not after it.**
	 *
	...
	/**
	 * ⚠️ **Optionality is a function of the position too, not only of the `?` in the spec.**
	 *
 ...
 *
 * ⚠️ **The count is restored on the way out, and that is the point.** A caller that wraps the
...
export function typeToZodBody(program: Program, type: Type): string {
	switch (type.kind) {
		...
		/**
		 * ⚠️ **A named union VARIANT is a type you can refer to, and it is not the union.**
		 *
